exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "416638032087613440")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign:Yapımcım mı sakın haa. Görmiyim bidaha.')
}})
  if(member.id === "408264865401012251")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Sen kimi etiketlediğini sanıyorsun he o etiket ağzına yapışsın emi endere sövemezsin`)
}})
if(member.id === "323381872392536064")return msg.channel.send({embed: {
  color: Math.floor(Math.random() * (0xFFFFFF + 1)),
  description: (`:no_entry_sign: Sen nasıl en iyi moderatörümüze sövüyorsun. Ayıp değilmi`)
 }})
 if(member.id === "454925587308347392")return msg.channel.send({embed: {
  color: Math.floor(Math.random() * (0xFFFFFF + 1)),
  description: (`:no_entry_sign: Nasıl yaparsın bunu en iyi kankam olmasada kankam ne sövüyon`)
 }})
  if(member.id === "329194657533919232")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Sen kime sövüyorsun kardanadamamı bak karışmam kardanadama sövemezsin!`)
}})
...
exports.help = {
  name: 'söv',
  description: 'İstediğiniz kişiye söversiniz',
  usage: 'söv'
 }
